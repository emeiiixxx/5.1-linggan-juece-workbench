...
export function buildNewProductPlanHtml({
  ...
  const safeClientBrand = clientBrand?.trim() ?? "";
...
<body>
  ...
  <main class="report">
    <section class="chapter cover" id="chapter-01" data-chapter>
      ${safeClientBrand ? `<div class="cover__brand">${escapeHtml(safeClientBrand)}</div>` : ""}
